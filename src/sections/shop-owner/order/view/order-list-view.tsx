...
import { ORDER_STATUS_OPTIONS, OrderResDT } from '../types/types';
...
export function OrderListView() {
  ...
  const [tableData, setTableData] = useState<OrderResDT[]>(orders);
  ...
  useEffect(() => {
    if (JSON.stringify(orders) !== JSON.stringify(tableData)) {
      setTableData(orders);
    }
  }, [orders, tableData]);
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Order', href: paths.shopOwner.order.root },
            { name: 'List' },
          ]}
          sx={{ mb: { xs: 3, md: 5 } }}
        />
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <OrderTableRow
                        ...
                        detailsHref={paths.shopOwner.order.details(row.id)}
                      />
                    ))}
...
function applyFilter({ inputData, comparator, filters, dateError }: ApplyFilterProps) {
  ...
  if (name) {
    inputData = inputData.filter(({ id, customer, total }) =>
      [id?.toString(), total?.toString(), customer?.username, customer?.email]
