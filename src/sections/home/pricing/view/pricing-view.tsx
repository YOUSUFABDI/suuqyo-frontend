...
export function PricingView() {
  ...
  return (
    <Container sx={{ pt: { xs: 5, md: 10 }, pb: 10 }}>
      ...
      <Box sx={{ mb: 5, position: 'relative', display: 'flex', justifyContent: 'center' }}>
      ...
      >
        {_pricingPlans.map((card) => (
          <PricingCard key={card.subscription} card={card} isYearly={isYearly} />
        ))}
      </Box>
    </Container>
  );
