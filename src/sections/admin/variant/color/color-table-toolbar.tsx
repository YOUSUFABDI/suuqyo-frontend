...
type Props = {
  onResetPage: () => void;
  filters: UseSetStateReturn<IColorsTableFilters>;
  options: {
    status: string[];
  };
};

export function ColorTableToolbar({ filters, options, onResetPage }: Props) {
  ...
  const handleFilterStatus = useCallback(
    (event: SelectChangeEvent) => {
      onResetPage();
      updateFilters({ status: event.target.value as IColorsTableFilters['status'] });
    },
    [onResetPage, updateFilters]
  );
